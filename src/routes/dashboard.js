...
router.get('/', requireAuth, async (req, res) => {
  ...
  const fleet = (await db.prepare('SELECT status, COUNT(*) AS n FROM cars GROUP BY status').all())
    .reduce((acc, row) => ({ ...acc, [row.status]: Number(row.n) }), {});

  const stats = {
    cars: Number((await db.prepare('SELECT COUNT(*) AS n FROM cars').get()).n),
    available: fleet.available || 0,
    rented: fleet.rented || 0,
    maintenance: fleet.maintenance || 0,
    customers: Number((await db.prepare('SELECT COUNT(*) AS n FROM customers').get()).n),
    activeRentals: Number((await db.prepare("SELECT COUNT(*) AS n FROM rentals WHERE status = 'active'").get()).n),
    overdue: Number((await db
